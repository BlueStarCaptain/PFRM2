...
export function PIDGenerator(Postnumber) {
    //Expect INT input
...
export function PID2VolCodes(PID) {
        //Expect String Hex PID Input
        const numVol = 4;
        const numIndexes = 5;
        const numCol = 10; // Define numCol
    ...
    var lotsofpish = "";
    var PIDHash = "";
    ...
    for (let t = 0; t < lotsofpish.length - 12; t += 12) {
        ...
        const calculation = (parseInt(PID.substring(0,6), 16) * VolumeSalt * chunkValueofTao).toString();
        ...
        if (parseInt(numVol.toString() + numIndexes.toString(), 10) > Nums10_99Array.length ) {
...
export function PID2DurCodes(PID) {
                //Expect String Hex PID Input
        const numDur = 4;
        const numIndexes = 5;
        const numCol = 10; // Define numCol
...
for (let t = 0; t < lotsofpish.length - 12; t += 12) {
    const chunk = lotsofpish.substring(t, t + 12);
    const chunkValueofTao = parseInt(chunk, 10) * 2;
    const calculation = (parseInt(PID.substring(0,6), 16) * DurationSalt * chunkValueofTao).toString();
    PIDHash += calculation.padStart(12, '0');
  }
  ...
  //console.log(PIDHash);
        ...
        if (parseInt(numDur.toString() + numIndexes.toString(), 10) > Nums10_99Array.length ) {
        }
...
//______________________________________________________________________________________________________________________________
//WORKS AND RUNNING 9-3-2023
export function PID2PostNumber(PID){
      // Check input to be correct
        if (
            typeof PID !== 'string' ||
            PID.length !== 7 ||
...
export function PID2ADMIN(PID) {
    ...
    if (allNumbersVolume.size > 0) {
        const sortedVolumeArray = Array.from(allNumbersVolume).sort((a, b) => a - b);
        ...
        if (sortedVolumeArray.length % 2 === 1) {
            ...
            VolPart = Math.ceil((sortedVolumeArray[middleIndex - 1] + sortedVolumeArray[middleIndex]) / 2);
        }
    } else {
    ...
    if (allNumbersDuration.size > 0) {
        const sortedDurationArray = Array.from(allNumbersDuration).sort((a, b) => a - b);
        ...
        if (sortedDurationArray.length % 2 === 1) {
            ...
            // If there's an even number of elements, take the average and round up
            DurPart = Math.ceil((sortedDurationArray[middleIndex - 1] + sortedDurationArray[middleIndex]) / 2);
        }
    } else {
...
/*
console.log("test 1")
const PID = "0000a2c"; // Corrected the quotes
console.log(generateAdminCode(PID));
*/
